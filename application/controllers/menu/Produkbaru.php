...
class Produkbaru extends CI_Controller {
	public function index() {
		$this->load->model('mstproduct_model');
		$data=array('title'=>'Semua order',
					'isi'  =>'menu/produkbaru/index',
					'list_product'=>$this->mstproduct_model->get_new()
				);
	...
	public function cari() {
		...
		$data=array('list_user' => $this->tbmusermodel->getuser(),
                    ...
                    'list_product' => $this->mstproduct_model->cari_new($this->input->post('cari')),
        ...
        $data = array(
            ...
            if (
            	$this->mstproduct_model->update($data, $idorder)) {redirect('menu/produkbaru');}
            else {redirect('menu/produkbaru/update/' . $idorder);}
        }
     ...
     public function delete($idorder) {
		...
		public function destroy() {
			...
			$idorder= $this->input->post('idorder');
        if (
        	$this->mstproduct_model->delete($idorder)) {redirect('menu/produkbaru');}
        else {
        	redirect('menu/produkbaru/delete/' . $idorder);}}
